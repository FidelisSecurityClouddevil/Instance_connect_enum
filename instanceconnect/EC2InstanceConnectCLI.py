...
from instanceconnect import __version__ as CLI_VERSION
from instanceconnect import ec2_util, key_publisher
...
class EC2InstanceConnectCLI(object):
    ...
    def __init__(self, instance_bundles, pub_key, cli_command, logger, aws_access_key, aws_secret, aws_session_token ):
        """
    ...
    def invoke_command(self):
        ...
        try:
            for bundle in self.instance_bundles:
                session = self._get_botocore_session(self.aws_access_key, self.aws_secret,self.aws_session_token, profile_name=bundle['profile'], region=bundle['region'])
    ...
    @staticmethod
    def _get_botocore_session( aws_access_key, aws_secret,aws_session_token, profile_name=None, region=None):
        """
        ...
        session.set_credentials(aws_access_key, aws_secret, aws_session_token )
