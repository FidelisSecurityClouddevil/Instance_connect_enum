...
class EC2InstanceConnectCommand(object):
    ...
    def __init__(self, program, instance_bundles, key_file, flags, program_command, logger, connect_directly):
        """
        ...
        self.connect_directly = connect_directly
    ...
    def get_command(self):
        ...
        command = '{0}  -o  "IdentitiesOnly=yes" -o "StrictHostKeyChecking no" -o PasswordAuthentication=no -q -i {1}'.format(self.program, self.key_file)
        ...
        # Target
        command = "{0} {1}".format(command, self._get_target(self.instance_bundles[0],self.connect_directly))
        ...
        if len(self.instance_bundles) > 1:
            command = "{0} {1}".format(command, self._get_target(self.instance_bundles[1],self.connect_directly))
    ...
    @staticmethod
    def _get_target(instance_bundle, connect_directly):
        """
        ...
        if instance_bundle.get('host_info', None):
            if connect_directly:
               ...
               target = "{0}@{1} 'exit'".format(instance_bundle['username'], instance_bundle['host_info'])
        ...
        if instance_bundle.get('file', None):
           if connect_directly:
              ...
              target = "{0}:{1} 'exit'".format(target, instance_bundle['file']).lstrip(':')
        return target
